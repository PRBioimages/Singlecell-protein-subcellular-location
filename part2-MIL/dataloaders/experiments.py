...
class RandomKTrainTestSplit:
    ...
    def get_dataloader(self, train_shuffle=True, tta=-1, tta_tfms=None):
        ...
        train_ds = MILDataset(df=self.train_meta, tfms=train_tfms,
                                 cfg=self.cfg, mode='train', p=self.pseodu)
        ...
        # valid_ds = MILDataset(df=self.valid_meta, tfms=val_tfms, cfg=self.cfg,
        #                          mode='valid', p=self.pseodu)
