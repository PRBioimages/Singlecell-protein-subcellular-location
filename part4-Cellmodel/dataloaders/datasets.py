...
class STRDataset(Dataset):
    def __init__(self, df, tfms=None, size=224, cfg: Config=None, mode='train'):
        self.df = df
    ...
    def __getitem__(self, idx: int):
        ...
        s = row["idx"]
        path = self.root + f'/{row["ID"]}_{s}.npy'
        img = np.load(path)
        if not self.cfg.transform.size == 224:
            img = cv2.resize(img, (self.cfg.transform.size, self.cfg.transform.size))

        if self.mode == 'train':
            if self.celllabel != 'imagelabel':
                cell_label = row[self.labael].values.astype(np.float)
            else:
                cell_label = self.df_img[self.df_img.ID.isin([row["ID"]])].reset_index(drop=True)
                cell_label = np.max(cell_label.loc[0:, self.cols].values.astype(np.float), axis=0)
            if self.tfms:
                return (
        ...
        if self.mode == 'valid':
            return (self.tensor_tfms(img), row[self.labael].values.astype(np.float))
