function [cell_feat, exit_code] = process_img(in_folder,out_folder,...
    resolution,color,extensions,pattern,mstype,seg_channels,steps,...
    run_partial_scans)
...
if(steps(1))
    ...
    for i=1:length(dirlist)
        ...
        label_subfiles{currind} = currdir;
        currind = currind+1;
    end
    ...
    if isempty(label_subfiles{1})
        pathparts = strsplit(image_path,'/');
        label_subfiles{1} = pathparts{end};
        
        
    end
    
    label_names = label_subfiles;
    
    number_labels = length(label_subfiles);
...
exit_code = zeros(1,length(label_subfiles));
for index = 1:length(label_subfiles)
%     if(step2)
        
        
        
        %set up the subfolder stuff
        curr_out_folder = fullfile([out_folder,filesep,label_subfiles{index}]);
        ...
        image_subdirectory      = [image_path, filesep, (label_subfiles{index}), filesep];
        ...
        %         image_path
        storage_subdirectory    = [processed_path, (label_subfiles{index}), filesep];
...
%         try
            ...
            %% Processing images
            image_subdirectory
            [nucfiles{index}, skipimage{index}] = preprocessImages(image_subdirectory,...
                base_naming_convention,curr_out_folder)
            ...
            %             return
            %         end
            %% segmentation and getting features
            [label_features{index}, imageids, feature_names, feature_computation_time, cell_seed, ...
                nucleus_seed,segskips,slf_names] = get_concatenated_region_features(...
                image_subdirectory, storage_subdirectory, base_naming_convention, ...
                label_names{index}, true, false, resolution);
            ...
            dir_png         = dir(fullfile([storage_subdirectory,filesep,'segmentation',filesep,'*',pngsuff]));
            ...
            for i = 1:length(dir_png)
                ...
                currpath = [storage_subdirectory,filesep,'segmentation',filesep,char(dir_png(i).name)];
            ...
            if sum(position_results(:))>0
                ...
                save(fullfile([curr_out_folder,filesep,'imageids.mat']),'imageids');
                csvwrite(fullfile([curr_out_folder,filesep,label_subfiles{index},'_features.csv']), cell_feat);
                
            elseif sum(sum(skipimage{:}>0))==length(dir_png) || sum(segskips>=1)==length(dir_png)
...
%         end
    ...
    if(steps(3))
        ...
        outpath = [curr_out_folder,filesep,label_subfiles{index},'_segmentation.png'];
