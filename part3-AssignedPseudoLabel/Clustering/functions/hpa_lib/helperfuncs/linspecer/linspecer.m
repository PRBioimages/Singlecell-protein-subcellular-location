...
%
%Edited by: 
%Devin P Sullivan - 21/07/2015 Changed brightness of colors slightly with
%the additions of fr,fy,fb to tune them. 
%%
...
set1JL = brighten(colorBrew2mat({[228, 26, 28];[ 55, 126, 184];[ 77, 175, 74];[ 255, 127, 0];[ 255, 237, 111];[ 166, 86, 40];[ 247, 129, 191];[ 153, 153, 153];[ 152, 78, 163]}'));
% set1 = brighten(colorBrew2mat({[ 55, 126, 184]*.95;[228, 26, 28];[ 77, 175, 74];[ 255, 127, 0];[ 152, 78, 163]}),.8);
set1 = brighten(colorBrew2mat({[ 55, 126*fy, 184];[228, 26, 28*fb];[ 77*fr, 175, 74*fb];[ 255, 127*fy, 0];[ 152*fr, 78, 163]}));
...
switch N
    ...
    case {10, 11, 12}
        if qualFlag==1 % force qualitative graphs
            lineStyles = set3(1:N)';
        elseif qualFlag==2
            lineStyles = cmap2linspecer(colorm(N,1));
...
frac=1; % Slight modification from colorbrewer here to make the yellows in the center just a bit darker
fb = 0.4;
fr = 0.6;
fy = 0.8;
...
if ~twosided
cmapp = [158, 1, 66*fb; 213, 62, 79*fb; 244, 109, 67*fb; 253, 174, 97*fb; 254, 224, 139*fb; 255*frac, 255*frac, 191*frac; 230*fr, 245, 152; 171*fr, 221*fy, 164; 102*fr, 194*fy, 165; 50*fr, 136*fy, 189; 94*fr, 79*fy, 162];
else
cmapp = [158, 1, 66*fb; 244, 109, 67*fb; 255, 255, 255; 255, 255, 255; 225, 225, 0; 230*fr, 245, 152; 171*fr, 221*fy, 164; 102*fr, 194*fy, 165; 50*fr, 136*fy, 189; 94*fr, 79*fy, 162];
end
