...
idx2 = find(hpatable.if_staining_intensity_id == 4 ...
    | hpatable.unspecific == 1);

idx3 = (1:1:length(hpatable.antibody))';

idx4 = zeros(size(idx3));
for i=1:length(idx3)
    if ~sum(idx3(i)==idx)
        idx4(i) = idx3(i);
    end
...
fname = 'data/metadata/hpaliststest.mat';
if exist(fname,'file')
    load(fname);
else
