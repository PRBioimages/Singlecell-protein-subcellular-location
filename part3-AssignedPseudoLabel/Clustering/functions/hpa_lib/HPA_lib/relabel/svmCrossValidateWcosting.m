function allweights = svmCrossValidateWcosting( tmpname, allfeatures, classlabels, antibodyids, idx_sda, c, g, Ninit, runs, costmin)
...
if ~isempty(tmpname) && exist(['./data/classification/',tmpname,'_',num2str(Ninit),'folds.mat'],'file')
   ...
   load(['./data/classification/',tmpname,'_',num2str(Ninit),'folds.mat'],'trainidx','testidx','protlabels','N','classes');
else
...
%     testind = testidx{i};
    ...
    nums = zeros(length(classes),1);
    for j = 1:length(classes)
        nums(j) = length(find(trainlabels==j));
    end
    nums = min(nums)./nums;
    nums = max(costmin(i),nums);
    costs = zeros(length(trainlabels),1);
    costs = nums(trainlabels); 

    [predict_label weights] = jl_svm_costing(c(i),g(i),costs,runs(i),i,...
...
%}
    ...
    predlabels(testind) = predict_label;
    allweights(testind,:) = weights;
end
